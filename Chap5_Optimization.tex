\chapter{Otimização da Banda de Guarda de Sistemas DDO-OFDM}
...
a potência da portadora óptica despediçada) e distorções não lineares
...
$B_{W}$ é necessário uma banda de guarda $B_G = B_{W}$ para garantir que não
...
Sabendo que muitas vezes, os parâmetros estão ligados de forma não linear,
tornou-se interessante o estudo e utilização de um algorítmo de otimização na
tentativa de encontrar valores razoáveis que melhoram o desempenho do sistema
dentro de uma vasta gama de parâmetros de configuração e suas faixas de
variações que compõem o sistema de transmissão.
...
% Otimização Mono-objetiva para Transmissão OFDM e Detecção Direta em
% \textit{Links} de Curto alcance (\textit{Short-Range})

% Abstract - O estudo apresentado neste artigo é dedicado no emprego de algorítmos
% genético na otimização de parâmetros do sistema OFDM com detecção direta em
% enlaces de curto alcance. Resultados de simulações de sistemas de transmissão na
% configuração \textit{back-to-back (B2B)} óptico utilizando fibras SSMF de 20 e
% 40km com parâmetros otimizados sugerem a aplicação da proposta de algorítmos
% mono-objetivos. Um sistema experimental com transmissão de 3.56 Gb/s através de
% 40 km de fibra SSMF sem compensação foi alcançado.

% \section{Introdução}
% Multiplexação por divisão de frequências ortogonais (\textit{Orthogonal
% Frequency Division Multiplexing} - OFDM) é atrativo para comunicações ópticas,
% principalmente devido a sua resistência a dispersão cromática em fibra óptica e
% largamente utilizado em sistemas de transmissão sem fio devido ao problema do
% multipercurso. OFDM óptico com detecção direta (DDO-OFDM) é um forte candidato
% para enlaces de curto alcance, redes de acesso e \textit{data centers}
% principalmente, devido a sua simplicidade na arquitetura do receptor.
% Entretanto, o OFDM gerado eletricamente pode ser uma técnica promissora para
% provedores de serviço de banda larga emergentes para suportar topologias sem
% fio, ao lado suas desvantagens de inserção da banda de guarda e ineficiencia
% espectral na qual requer metade da potência transmitida para a portadora
% óptica[6].
% 
% O extenso número de parâmetros envolvidos no desempenho na transmissão de sinais
% OFDM em sistemas ópticos de detecção direta resulta no projeto potência do sinal
% transmitido, sensibilidade do receptor, robustes contra dispersão cromática na
% fibra (CD) e eficiência espectral geral do sistema [7-10]. Foi demonstrado em
% [7] que uma sensibilidade ótima depende da média de potência do sinal elétrico e
% do ponto de polarização do modulador externo MZM. Um índice de modulação óptica
% ótimo necessita ser encontrado para superar a potência óptica do sinal
% (especialmente a potência óptica desperdiçada da portadora) e as distorções não
% lineares [8]. É fato que a eficiência global do sistema de detecção direta é
% afetada pela banda de guarda na qual é necessária para previnir da interferência
% do batimento do sinal óptico (signal-to-signal beat interference (SSBI) ou
% interferência da mistura das subportadoras (subcarries-to-subcarriers mixing
% interference - SSMI) [7],[10]. Aqui também mostra que o projeto deve reduzir a
% banda de gurada, aumentando a eficiência espectral, superando as interferências
% não desejadas [7-9]. Além disso, uma ênfase entre o intevalo de guarda total
% denominado por prefíxo cíclico (CP) e uma boa tolerância a dispersão cromática
% da fibra em uma alta sensibilidade na recepção são levados em consideração [11],
% [12]. Assim, o uso de um algoritmo de optimização, parece ser razoável a fim de
% alcançar o melhor desempenho dos referidos sistemas de comunicação óptica dentro
% de uma vasta gama de parâmetros de configuração.

% Neste capítulo será enfatizado o desempenho aprimorado de sistemas DDO-OFDM com
% enlaces de curto alcance com parâmetros otimizados por algoritmos genéticos 
% (Genetic algorithms - GA) mono-objetivo. Também será mostrado a transmissão
% experimental otimizada com um sinal OFDM de dupla faixa lateral com $R_b =$ 3.56
% Gb/s por 40 km de fibra sem compensação.

% \section{Sistema OFDM Óptico com Detecção Direta}
% II - DIRECT-DETECTION OPTICAL OFDM SYSTEM
% 
% O sistema OFDM óptico de comunicação por detecção direta e seus pontos mais
% importantes de projeto relacionados a um extenso número de parametros envolvidos
% no seu desempenho são descritos nesta seção.

% Descrição geral
% O estudo apresentado neste capítulo é dedicado ao emprego de algoritmo genético
% para otimização de parâmetros do sistema OFDM com detecção direta em enlaces de
% curto alcance a fim de obter melhor desempenho. Resultados de simulações de
% sistemas de transmissão na configuração \textit{back-to-back (B2B)} óptico
% utilizando fibras SSMF de 20 e 40 km com parâmetros otimizados sugerem a
% aplicação da proposta de algorítmos mono-objetivos. Além disso, com o sistema
% experimental DDO-OFDM II utilizando 40 km de fibra SSMF é alcançado uma taxa de
% transmissão de 3.56 Gb/s.
% 
% \subsection{Modelo do Sistema}
% A Figura 1 mostra o diagrama de blocos do modelo do sistema considerado para as
% transmissões de sinais em enlaces de curto alcances. Um fluxo de dados binários
% são convertidos de serial para paralelo e antes do mapeamento para símbolo
% 
% Um fluxo de dados binários é convertido de serial para paralelo e de pois de
% mapear em súmbolo o vetor de sinal complexo resultante composto pelo fluxo de
% baixa taxa de bit são modulados em subportadoras ortogonais realizado pela
% transoormada inversa de fourier (IFFT). Antes da conversaõ paralelo para serial
% o prefíxo cíclico é adicionado para eliminar imparidades causadas pela dispersão
% da fibra. O sinal OFDM complexo na banda base é convertido para sinal analógico
% usando um conversor digital-para-analógico (DAC) e UPCONVERTER pela portadora RF
% antes de direcionar para o modulador óptico de um braço.
% Um modulador Mack-Zehnder (MZM) convencional é usado para modular o sinal OFDM
% na portadora óptica fornecida pela onda contínua de luz do laser que opera no
% modo CW. Os sinais de banda lateral única (single-side-band - SSB) gerados por
% um filtro óptico (OF) são então transmitidas por amplificação e sem compensação
% por dispersão em fibras mono modo padrão (SSMF). Depois de filtrado por uma
% banda passante, a intensidade óptica é detectada diretamente por um único foto
% diodo (PD). Antes da detecção OFDM, a saída do fotodiodo é digitalizada usando o
% conversor ADC e então \textit{DOWNCONVERTED} para a banda base pelo misturador
% IQ basedo em \textit{software}. Mediante a remoção do prefíxo cíclico, o sinal
% OFDM é demultiplexado pela transformada rápida de Fourier (FFT). Finalmente, os
% dados binários são recuperados pelo demapeamento e equalização do sinal. A fase
% e amplitude de cada subportadora são corrigidos por equalização pela função
% transferência de one-tap estimada usando símbolos de treinamento. Um sistema
% perfeito de sincronismo é assumido nas simulações conduzidas, para simplicidade.

\subsection{Declaração do Problema}

% Sabe-se que o desempenho do sistema óptico de detecção direta estudado depende
% de um extenso número de parâmetros, relacionados de formas linear e não linear,
% que envolvem as características dos sinais OFDM tais como, ponto de polarização
% do modulador óptico, deficiências na fotodetecção direta, etc. Estudos teóricos
% e experimentais apresentados em [7], [8] e [9] revelam pontos relevantes do
% projeto entre potência do sinal transmitido, sensibilidade de recepção, robustez
% contra CD e eficiência espectral global do sistema. Excluindo o prefíxo cíclico
% em uma configuração óptica B2B, um único símbolo OFDM de um sinal DDO-OFDM real
% pode ser escrita como  [7]-[12].
...
\left \{ \mathfrak{R_e} [X_k] \cdot cos(2\pi k \Delta_f t) + \mathfrak{I_m}
 [X_k] \cdot (2 \pi k \Delta_f t) \right \},
...
de $\Delta_f$. De acordo com \cite{Rosenkranz09}, o campo óptico na saída de um
...
% onde ${\sigma_s}^2$ é a potência média e $X_k$ representa os símbolos complexos
% de dados transmitidos pelas $N_FFT$ subportadoras espaçadas na de frequência de
% $\delta$f. De acordo com [7], o campo óptico na saida de um MZM caracterizado
% pela sua tensão de chaveamento $V_\pi$ e polarizado por $V_bias$ (see Figure 2)
% é um sinal de banda passante centralizado na frequência $f_0$ dado por

\begin{equation}
E_{MZM} = cos \left [ \frac{\pi}{2} \frac{s(t) + V_{bias}}{V_\pi} \right ] \cdot
\sqrt{2P} cos(2\pi f_0 t)
\label{Eq:mzm2}
\end{equation}
...
potência média do sinal OFDM e o ponto de polarização do MZM aliam uma
importante regra no desempenho do sistema DDO-OFDM. Um olhar mais atento na
Figura \ref{Funcao_tranf_MZM} revela que a polarização do MZM no seu ponto de
quadratura significa que o modulador óptico opera na região de máxima
linearidade. Embora isso resulte em um aumento na amplitude do sinal elétrico na
entrada do MZM, metade da potência óptica é desperdiçada pela portadora óptica.

Polarizando o modulador logo acima da tensão nula (por exemplo $V_{bias} = 0.8
\cdot V_\pi$) reduz a perda de potência da portadora e ao mesmo tempo que
introduz termos de alta ordem provindos da função transferência do MZM.
% [7]-[9].
...
\section{Otimização Mono-Objetiva via Algoritmo Genético}
...
aumentou. Percebe-se pelos resultados da Tabela \ref{Tab:4}, que $V_{bias}$ em
...
\approx -14.9$, mostrada na Figura \ref{Fig:6}, confirmam a adequação da
...
Uma comparação justa entre o sistema OFDM óptico de modulação por intensidade
...
eficiência espectral) que cheram aos 50\%.

% \section{Conclusões} %VERIFICAR!
% O uso de algoritmos genéticos na otimização de parâmetros de sistemas ofdm
% óptico de detecção direta foi proposto. 
% 
% Tem sido proposto o uso de algoritmos genéticos nos parâmetros de otimização de
% sistemas OFDM óptico de detecção direta. Transmissão experimental de um sinal de
% 3.56 Gb / s de banda lateral dupla OFDM sobre 40 quilômetros de fibra
% descompensada , com parâmetros otimizados por um GA em uma formulação do
% problema inicial, sugere o aparelho do algoritmo genético mono- objetivo , a fim
% de aumentar o desempenho do sistema. Os resultados da simulação de um sistema de
% comunicação óptica DDO -OFDM com parâmetros otimizados pelo mesmo algoritmo
% genético aplicado a um novo fitness que considera tanto o desempenho e
% eficiência de espectro , mostra que o gab freqüência tipicamente reservado em
% sistema OFDM óptico de intensidade modulada anterior pode ser reduzido. A SE
% geral do sistema aumenta em simulações conduzidas que consideram 4, 8 e 16-QAM
% mapeamento subportadora na configuração B2B e através de 20 e 40 km de SSMF .
% Comparado com o convencional DDO - OFDM , o sistema optimizado demonstrou uma
% economia da largura de banda entre 20 e 33 \% com ganhos ligeiros em OSNR .
% 
% In addition, the formulated problem suggests the appliance of a multi -objective
% algorithm in future development of high speed optical DDO-OFDM in both  short 
% and long-haul links.  Instabilities  in the proposed  optimization  model 
% related  to  parameters  variations  such  as  MZM  bias  voltage,  optical
% carrier-to-signal  power  ratio  (CSPR)  and  OFDM  signal  peak-to-average 
% power ratio  (PAPR)  are relegated to future work.  Particle swarm, ant colony
% and variable neighborhood search are examples of  metaheuristic  that  can  be 
% applied  in  the  formulation  to  further  improve  the  aforementioned
% drawbacks.
% 
% 
% 
...
% 
% Sabe-se que o desempenho do sistema óptico de detecção direta estudado depende
% de um extenso número de parâmetros que envolvem as características dos sinais
% OFDM, tais como ponto de polarização do modulador óptico, deficiências na
% fotodetecção direta, etc [7]-[12].
% 
% FIGURA 1.
% 
% Estudos teóricos e experimentais apresentados em [7], [8] e [9] revelam pontos
% relevantes do projeto entre potência do sinal transmitido, sensibilidade de
% recepção, robustez contra CD e eficiência espectral global do sistema. Como
% descrito em [7], excluindo o prefíxo cíclico (b2b configuração óptica), um único
% símbolo OFDM de um sinal DDO-OFDM real pode ser escrita como
% 
% s(t) = \ldots
% 
% onde ${\sigma_s}^2$ é a potência média e $X_k$ representa os símbolos complexos
...
% Emzm = cos\ldots
% 
% para $P$ a potência do \textit{laser} de sinal CW conectado na entrada óptica do
% MZM.
% 
% É possível notar que a partir das equações (1) e (2) que a potência média do
% sinal OFDM e o ponto dep olarização do MZM aliam uma importante regra no
% desempenho do sistema DDO-OFDM. Um olhar mais atento na figura 2 revela que a
% polarização do MZm no seu ponto de quadratura significa que o modulador óptico
% opera na região de máxima linearidade. Embora isso resulte em um aumento na
% tensão raiz média quadrática da amplitude ($V_{in_RMS}$) do sinal elétrico na
% entrada do MZM, metade da potência óptica é desperdiçada pela portadora óptica.
% Polarizando o modulador logo acima da tensão nula (por exemplo $V_{bias} = 0.8
% \cdot V_\pi$) reduz a perda de potência da portadora e fornece um desempenho
% otimizado no ruído no mesmo tempo que introduz termos de alta ordem provindos da
% função transferência do MZM [7]-[9].
% 
% FIGURA 2.
% 
% Conduzindo a simulação de Monte Carlo (MC) com um sistema DDO-OFDM em b2b
...
% consideráveis.
% 
% Alternativamente, o sinal óptico dado por (2) pode ser expresso como
% 
% \begin{equation}
% E_{MZM}(t) = A + e^{j2\pi(f_0+B_G)t} \cdot \sum_{k=1}^{N_{FFT}} X_k
% e^{j2\pi(k\Delta f)t}
...
% onde A é uma constante representando a portadora óptica e $B_G$ a banda de
% guarda entre a portadora óptica e o sinal OFDM [10]. Em condições de B2B, a
% fotocorrente na saída do fotodiodo é computada pelo quadrado de $|E_{MZM}(t)|^2$
% resultando em
% 
% \begin{equation}
% i_{pin}(t) \propto |E_{MZM}(t)|^2 = |A|^2 + 2\Re \left[ A \cdot
% e^{j2\pi(f_0+B_G})t \right] + \underbrace{\left[ \sum_{k=1}^{N_{FFT}} X_k e^{j2\pi(k \Delta
% f)t} \right ]}_{OFDM signal}
% + \underbrace{\sum_{k=1}^{N_{FFT}}\sum_{k=1}^{N_{FFT}} X_k X_l^\ast
% e^{j2\pi[(k-1)\Delta f t]}}_{SSBI or SSMI}
% \end{equation}
% 
% onde $\ast$ demonstra a operação complexo conjugado do termo. Resultado do
% batimento das subportadoras OFDM, o termo de distorção de intermodulação por
% baixa frequência IMD que aparece em (4) degrada o sinal. As penalidades dessas
% deficiências denominam na literatura por interferência por batimento sinal-sinal
% (signal-to-signal beating interferences - SSBI ou signal-to-singnal mixing
% interferences - SSMI) podem ser minimizadas se uma banda de guarda $B_G$ for
% alocada [9], [10]. Aqui, uma abordagem entre degradação não linear do sinal e
% eficiência espectral do sistema são levadas em conta. Vale notar que, em
% sistemas DDO-OFDM convencionais uma banda $B_G = B_W$ é recomendada.
% 
% Outra abordagem que existe em sistemas ópticos OFDM são relacionados com a
% inserção do prefíxo cíclico no início de cada sinal transmitido. Uma informação
% redundante denominada por $\epsilon_g = T_g/(T_s-T_g)$ para $T_g$ o intervalo
% de guarda e $T_s$ o tempo total do símbolo OFDM, introduz um cabeçalho no
% sistema de comunicação. Aqui, um curto prefíxo cíclico escolhido deve minizar
% ou até mesmo eliminar todos efeitos causados pela interferência inter
% simbólica (ISI) no sistema e, por outro lado compensa o deslocamento na
% sincronização do símbolo. Uma boa estimativa para a eficácia deste intervalo de
% guarda é
% 
% 
% \begin{equation}
...
% 
% onde $D$ é a quantidade total de dispersão cromática, $c$ é a velocidade da luz
% e $f_0$ a frequência da portadora óptica [11], [12]. Obtida de (5), o mapa
% contornado mo
...
% As representações reais permitem um algoritmo evolucionário a operar em uma
% população de vetores em um domínio de busca limitado $\Omega$ incluído em
% $\textbf{R}^n$. Vamos assumir que, dada uma geração, os indivíduos \textbf{X} de
% uma população são gerados em um domínio de busca de acordo com a
% distribuição probabilidade caracterizada pela densidade $p(x)$, onde $x$ é um
% ponto em $\Omega$. Então a distribuição tem uma expectativa:
% 
% \begin{equation}
% E(X) =  \int\limits_\Omega xp(x) dx,
% \label{Eq:meanEq}
% \end{equation}
% 
% e a variância:
% 
% \begin{eqnarray}
% V(X) = \int \limits_\Omega x^2p(x) dx - E(X^2)
% \label{Eq:varEq}
% \end{eqnarray}
% 
% Se $\lambda$, o tamanho da população dos descendentes, é suficientemente
% grande, estes valores são abordados pela expectativa empírica:
% 
% \begin{eqnarray}
% V = \frac{\sum\nolimits_{i=1}^\lambda x_i^2}{\lambda} - E^2
% \label{Eq:emp}
% \end{eqnarray}
% 
% A variância empírica pode ser considerada como uma medida da diversidade na
% população. Se for nula, então todos os indivíduos estão no mesmo ponto em
% $\Omega$. Ao adotar uma analogia mecânica, $\widehat{E}$ é o centroide da
% população, enquanto $\widehat{V}$ é o momento de inércia do centróide,
% atribuindo para cada indivíduo uma unidade de massa. Isto é interessante para
% avaliar esses valores após a aplicação dos operadores de variação.
% 
% \subsubsection{Cruzamento}
% Vamos considerar dois pontos $x$ e $y$ no espaço $\mathbb{R}^n$ correspondendo a
% dois indivíduos selecionados para gerar os descendentes. Depois da aplicação do
% operador cruzamento, um ou dois descendentes $x'$ e $y'$ são escolhidos
% aleatóriamente, de acordo com a distribuição probabilidade na qual depende de
% $x$ e $y$.
% 
% \textit{Cruzamento por intercâmbio de componentes:}
% 
% 
% 
