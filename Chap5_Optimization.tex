...
\chapter{Otimização da Eficiência Espectral de Sistemas DDO-OFDM}
...
\section{Introdução}
Após o desenvolvimento da plataforma de transmissão e a realização de testes do
sistema óptico experimental com detecção direta, foi observado que a variação de
alguns do extenso número de parâmetros que compõem as características de
formação de sinais em sistemas DDO-OFDM, afeta significativamente a qualidade da
transmissão. Tais parâmetros são responsáveis pela potência do sinal na entrada
da fibra, pela sensibilidade do receptor, pela robustez contra dispersão
cromática da fibra e pela eficiência espectral do sistema.

Sabe-se que a sensibilidade no receptor depende da potência média do sinal
elétrico e do ponto de polarização $V_{bias}$ do modulador externo MZM
\cite{Rosenkranz09}, \cite{WilnnerJLT09_02}. Além disso, um índice de modulação
óptica (\textit{Optical Modulation Index} - OMI) deste modulador deve ser
otimizado de forma a equilibrar o compromisso entre a potência óptica do sinal
(em especial a potência da portadora óptica desperdiçada) e as distorções não
lineares introduzidas pelo próprio MZM \cite{WilnnerJLT09_02}. Sabe-se também
que a eficiência global do sistema DDO-OFDM é afetada pela banda de guarda $B_G$
necessária para evitar a interferência de batimendo do sinal
(\textit{signal-to-signal beat interference} - SSBI ou
\textit{subcarries-to-subcarries mixing interference} - SSMI) presente na
recepção óptica. Neste quesito em especial, a literatura diz que, para uma banda
efetiva de transmissão $B_{W}$ é necessária uma banda de guarda $B_G = B_{W}$
para acomodar as interferências SSBI, concentradas próximas à portadora óptica
\cite{Rosenkranz09}.
A redução desta banda de guarda torna-se tarefa essencial dos sistemas aqui
retratados de forma a aumentar a eficiência espectral dos mesmos.

% Entretanto, foi observado que as interferências por batimento se concentravam
% próximos às baixas frequências motivando uma tentativa de diminuição desta banda a fim de
% maximizar o uso do espectro, junto ao equilíbrio entre o intervalo de guarda
% total, com o objetivo de tolerar a dispersão cromática da fibra.

Sabendo que muitas vezes os parâmetros de projeto estão interligados de forma
não linear, tornou-se interessante neste estudo a utilização de um algoritmo de
otimização na tentativa de encontrar valores que aumentam a eficiência
espectral dos sistemas.

Este Capítulo enfatiza o aumento da eficiência espectral de sistemas DDO-OFDM em
enlaces de curto alcance com parâmetros otimizados por um algoritmo Genético
(\textit{Genetic algorithm} - GA) mono-objetivo. A eficiência da otimização é
mostrada perante a transmissão experimental de um sistema DDO-OFDM otimizado,
a uma taxa de 1.83 Gb/s e 40 km de fibra SSMF sem compensação óptica da
dispersão cromática.
...
detecção direta estudado depende de um extenso número de parâmetros que estão
diretamente relacionados com a polarização do modulador óptico, com deficiências
na fotodetecção, entre outros. Estudos teóricos e experimentais apresentados em
\cite{Rosenkranz09}, \cite{WilnnerJLT09_02} e \cite{art:peng} revelam pontos
relevantes de projeto entre potência do sinal transmitido, sensibilidade de
recepção, robustez contra CD e eficiência espectral global do sistema. Excluindo
o prefíxo cíclico, um único símbolo OFDM pode ser escrito como% [7]-[12].
\begin{equation}
...
de dados transmitidos pelas $N_{FFT}$ subportadoras espaçadas com frequência de
$\Delta f$. De acordo com \cite{Rosenkranz09}, o campo óptico na saída de um MZM
caracterizado pela sua tensão de chaveamento $V_\pi$ e polarizado por $V_{bias}$
é um sinal de banda passante centralizado na frequência $f_0$ e pode ser dado
por
\begin{equation}
...
padrão $\sigma_s$ dado através do índice de modulação $OMI =
(V_{in})_{RMS}/V_\pi$.
Um olhar mais atento na Figura \ref{MZM_transferFunction} revela que a
polarização do MZM no seu ponto de quadratura mostra que o modulador óptico
opera na região de máxima linearidade. Embora isso resulte em um aumento na
amplitude do sinal elétrico na entrada do MZM, metade da potência óptica é
desperdiçada pela portadora óptica.
Polarizar o MZM acima do ponto de quadratura proporciona maior potência óptica
da portadora na saída do modulador. Em contrapartida, entra-se numa região de
não linearidade caso ocorram picos do sinal transmitido, ao polarizarmos o MZM
na zona de máxima transferência de potência da curva característica. Já
polarizando o modulador abaixo do ponto de quadratura e logo acima da tensão
nula, reduz a perda de potência pela portadora óptica, ao mesmo tempo em que são
introduzidos termos de alta ordem provindos de não linearidades dado também pela
curva da função transferência do MZM.

% Já polarizando o modulador logo acima da tensão nula (por exemplo $V_{bias} =
% 0.8 \cdot V_\pi$) reduz a perda de potência da portadora e ao mesmo tempo que
% introduz termos de alta ordem provindos da função transferência do MZM. Um
% olhar mais atento na Figura \ref{Funcao_tranf_MZM} revela que a polarização do
% MZM no seu ponto de quadratura significa que o modulador óptico opera na
% região de máxima linearidade. Embora isso resulte em um aumento na amplitude
% do sinal elétrico na entrada do MZM, metade da potência óptica é desperdiçada
% pela portadora óptica [7]-[9].

\begin{figure}[h]
	\begin{center}
		\includegraphics[scale=0.49]{Figures/MZM_Curve_cap5.pdf}
		\caption{Função característica do modulador óptico Mach-Zehnder e pontos de
		polarização}
		\label{MZM_transferFunction}
	\end{center}
\end{figure}
...
% ( e conseguentemente da potência média ${\sigma_s}^2$) do sinal OFDM. Em
% adição, a análise da dispersão inserida e da taxa de erro de bit (BER) versus
% a relação sinal ruido óptica (OSNR) mostra na Figura 3b para o melhor valor de
% OMI para cada ponto de polarização descrito na figura 3a, também são pontos
% consideráveis.

Alternativamente, o sinal óptico dado por (\ref{Eq:mzm2}) pode ser expresso como
\begin{equation}
...
onde $A$ é uma constante representando a portadora óptica e $B_G$ a banda de
guarda entre a portadora óptica e o sinal OFDM. Em uma configuração do tipo
\textit{back-to-back} (B2B), a fotocorrente na saída do fotodiodo é computada
pelo quadrado de $|E_{MZM}(t)|^2$ resultando em
% \begin{eqnarray*}
%  \lefteqn{w+x+y+z = }\\
%    && a+b+c+d+e+\\
%    && {}+f+g+h+i
...
\end{eqnarray}
onde $\ast$ denota a operação complexo conjugado do termo. Resultado do
...
penalidades inseridas pelas interferências SSBI podem ser minimizadas se uma
banda de guarda $B_G$ for alocada. Aqui, uma abordagem entre degradação não
linear do sinal e eficiência espectral do sistema são levadas em conta. Vale
notar que, em sistemas DDO-OFDM convencionais uma banda $B_G = B_W$ é
recomendada, conforme ilustra a Figura \ref{Fig:BGa}. Durante os procedimentos
experimentais descritos no Capítulo \ref{Cap:4}, notou-se que a banda de guarda
$B_G$ não necessariamente deveria seguir esta igualdade. Sendo assim, qual valor
de banda de guarda deve ser atribuída em aplicações em que sinais OFDM são
transmitidos em enlaces de curto alcance em sistemas ópticos IMDD? Ressalta-se
que esta banda de guarda deve considerar o compromisso entre a eficiência
espectral e a inserção de produtos de intermodulação, mantendo-se o desempenho
ou minimizando as possíveis penalidades oriundas da redução da banda de guarda.

\begin{figure}[h]
        \centering
        \begin{subfigure}[b]{0.9\textwidth}
                \includegraphics[width=\textwidth]{Figures/Graph_BG_Chap5.pdf}
                \caption{Inserção da Banda de Guarda no Sistema DDO-OFDM;}
                \label{Fig:BGa}
        \end{subfigure}
        ~	%add desired spacing between images, e. g. ~, \quad, \qquad etc.
			%(or a blank line to force the subfigure onto a new line)
        \begin{subfigure}[b]{0.9\textwidth}
                \includegraphics[width=\textwidth]{Figures/Graph_BG_Chap5-b.pdf}
                \caption{Espectro elétrico esperado com parâmetros
                otimizados.}
                \label{Fig:BGb}
        \end{subfigure}
        \caption{Variação proposta da banda de guarda do sistema DDO-OFDM.}
        \label{Fig:BG}
\end{figure}
A Figura \ref{Fig:BGb} ilustra o espéctro elétrico original com os produtos de
intermodulação em uma banda menor que $B_W$ e a evolução para o espéctro
elétrico do sinal deslocado para $f'_c$ que resulta em uma menor frequência
máxima do sinal ${B'}_c$. Um parâmetro que afeta bastante o desempenho citado
está relacionado à inserção do prefíxo cíclico no início de cada sinal
transmitido.
Uma redundância dada por $\epsilon_g = T_g/(T_s-T_g)$, para $T_g$ o intervalo de
guarda e $T_s$ o tempo total do símbolo OFDM, é introduzida no ínicio de cada
sinal OFDM. Um curto prefíxo cíclico escolhido deve minimizar ou até mesmo
eliminar todos os efeitos causados pela interferência intersimbólica (ISI) no
sistema que, por outro lado compensa o deslocamento na sincronização do símbolo.
Uma boa estimativa para a eficácia deste intervalo de guarda é
\begin{equation}
...
Entretanto, como trata-se de enlaces de curto alcance, efeitos causados pela
dispersão cromática são mínimos podendo diminuir o $I_G$ porém, não
eliminando-o, em virtude de possuir um sinal cíclico que viabilize uma abordagem
que usa a convolução circular para melhorar o entendimento da eficiência da
equalização de um atraso (\textit{one tap equalizer} nesses sistemas
\cite{Book:DSP_Mitra}. A Figura \ref{Fig:Graph_Ig-a} mostra curvas para uma dada
distância, valores de tempo de símbolo versus tempo de guarda. A Figura
\ref{Fig:Graph_Ig-b} demonstra a influência do intervalo de guarda na qualidade
da recepção do sinal OFDM.
\begin{figure}[h]
        \centering
        \begin{subfigure}[c]{0.69\textwidth}
                \includegraphics[width=\textwidth]{Figures/Graph_Ig_Cap5-a.png}
                \caption{Relação de valores Tempo de símbolo versus Tempo de
                guarda para uma mesma distância;}
                \label{Fig:Graph_Ig-a}
        \end{subfigure}
        %~	%add desired spacing between images, e. g. ~, \quad, \qquad etc.
			%(or a blank line to force the subfigure onto a new line)
        \begin{subfigure}[c]{0.30\textwidth}
                \includegraphics[width=\textwidth]{Figures/Graph_Ig_Cap5-b.png}
                \caption{Constelações com dois valores de tempo de guarda.}
                \label{Fig:Graph_Ig-b}
        \end{subfigure}
        \caption{Influência do intervalo de guarda no projeto de sistemas
        DDO-OFDM.}
        \label{Fig:Graph_Ig}
\end{figure}

Por fim, a última variável escolhida para o processo de otimização provê um
multiplicador da taxa de amostragem do sinal, dada por $n_{samp}$. A taxa de
amostragem é um parâmetro que se relaciona diretamente com a frequência máxima
do sinal. As portadoras de maiores frequências possuem menos pontos por ciclo
criando distorções na recuperação do sinal. O aumento da taxa de amostragem do
sinal pode melhorar a qualidade do sinal transmitido porém, exige maior
capacidade de processamento no lado do transmissor. Um valor aceitável para
$n_{samp}$ é buscado no algoritmo de otimização para obter melhores resultados
na tranmissão.
% Obtida de \ref{Eq:dispersao}, o mapa de contornos mostrado na Figuraontornado
% mo FALTA ENFATIZAR AS COMPONENTES DE INTERMODULAÇÃO PROVENIENTES DO BATIMENTO
% DO LASER NA INTRODUÇÃO.
...
Otimização mono-objetiva significa otimizar um problema bem definido sujeito a um conjunto 
de restrições, a partir de uma única função objetiva.
...
banda de guarda óptica, $GI$ ou $I_G$ é proporcional ao $CP$, $n_{samp}$ é um
fator de super amostragem, $V_\pi$ é a tensão de chaveamento do MZM, $B_W$ a
largura de banda do sinal de banda lateral (excluindo $B_G$) e $X$ é um número
inteiro de tal modo que $X_{min} \leq X \leq X_{max}$.
\begin{center}
\begin{table}[h]
\centering
...
\section{Técnica de Imposição da OSNR}
De maneira diferente do arranjo experimental, efeitos de ruído de emissão espontânea
de amplificadores (\textit{Amplified Spontaneous Emission} - ASE) como fonte de
ruído óptico predominante foram simulados por um gerador de ruído aleatório, conforme 
ilustra a Figura \ref{Fig:StructureSimulated}. 
Assim sendo, o ruído inserido ao sinal ótico, após a propagação na fibra é
ponderado conforme uma relação sinal-ruído óptica imposta.
% ASE nao sistema proposto para a otimização foi incluído um amplificador óptico
% ideal (AO) com a inserção de ruído gaussiano antes do fotodetector. Embora a
% densidade espectral de potência do ruído da emissão espontânea amplificada tem
% uma dependência com a frequência que varia em toda banda C (na qual possui
% largura de 4400 GHz) \cite{Book:Erbium}, a largura de banda dos sinais
% utilizados nesta dissertação são pequenas, menores que 50 GHz, o que pode-se
% considerar que a densidade espectral de potência é constante com a largura de
% banda do sinal.

\begin{figure}[h]
	\begin{center}
		\includegraphics[width=\textwidth]{Figures/DDO-OFDM_II_Simulated_blockDiagram_pt-br.pdf}
		\caption{Meio físico de propagação dos sinais propostos.}
		\label{Fig:StructureSimulated}
	\end{center}
\end{figure}

O ruído usado nas simulações é modelado como um ruído Gaussiano branco
aditivo(\textit{Additive White Gaussian Noise} - AWGN) \cite{MT:Filipe}. A
Figura \ref{Fig:Graph_OSNR} ilustra a metodologia de inserção de ruído aos
sinais ópticos, para $P_S$ a potência de sinal, $P_N$ a potência de ruído
gerada, $B_W$ a banda do sinal, $B_{Sim}$ a banda de simulação e $B_{Ref}$ uma
banda de referência. Portanto, a OSNR sendo a razão das potências ópticas do
sinal com o ruído pode ser dada por
\begin{equation}
OSNR = \frac{P_S}{P_N}.
\label{Eq:OSNR_fato}
\end{equation}
...
\begin{figure}[h]
	\begin{center}
		\includegraphics[width=0.8\textwidth]{Figures/Graph_OSNR.pdf}
		\caption{Densidade espectral de potência óptica com ruído adicionado.}
		\label{Fig:Graph_OSNR}
	\end{center}
\end{figure}

Mais precisamente, a relação sinal ruído óptica (\textit{Optical
Signal-Noise-Ratio} - OSNR) é definida como
\begin{equation}
OSNR = \frac{P}{2B_{Ref}N_{ASE}},
\label{Eq:OSNR1}
\end{equation}
onde $P$ é a potência média do sinal total, a banda de referência normalmente
escolhida como $0.1$ nm, ou $12.5$ GHz em $1550$ nm e $N_{ASE}$ é a densidade de
potência espectral da ASE em cada eixo de polarização óptica. O código que
implementa esta técnica de imposição da OSNR na simulação está descrito no Anexo
\ref{AppendixB}.

% Outra quantidade comumente utilizada em comunicações digitais e a SNR por bit
% \cite{Book:DigiComm_Proak}. Esta, é definida como a razão $\frac{E_b}{N_0}$,
% onde $E_b$ é a energia por bit e $N_0$ é a densidade espectral de potência de
% ruído. A relação entre SNR e OSNR é dada por
% \begin{equation}
% OSNR = \frac{pR_S}{2B_{Ref}} \cdot SNR,
% \label{Eq:OSNRxSNR}
% \end{equation}
% onde $p=1$ para um sinal com polarização simples, neste caso trabalhado, e $p =
% 2$ para um sinal de polarização multiplexada e $R_S$ é a taxa de símbolo. É
% possível notar que, a relação entre OSNR e SNR depende somente da informação da
% taxa de símbolo e independe se o sinal possui polarização multiplexada ou não
% \cite{art:capacity_fiber}.

% Portanto, conhecendo-se a SNR e a largura de banda de referência, assim como a
% taxa de símbolos por segundo, é possível estimar a OSNR.

\section{O Algoritmo Genético Aplicado ao Problema}
Utilizou-se neste trabalho o algoritmo genético com codificação de número real
...
Examina-se nesta seção o desempenho da heurística desenvolvida para aumentar a
eficiência espectral de sistemas DDO-OFDM em enlaces de curto alcance com
parâmetros otimizados conforme proposto pelo algoritmo genético mono-objetivo.
Os resultados de simulações e a devida comprovação experimental através da
transmissão de um sistema DDO-OFDM otimizado, a taxa de $R_b = 1.83$ Gb/s em
condições de B2B, 20 e 40 km de fibra SSMF sem compensação óptica de dispersão
são discutidos. Além disso, realiza-se aui uma discussão acerca da formulação do
problema que considera a maximização da eficiência espectral do sistema como um
todo.
...
A fim de otimizar parâmetros críticos foram conduzidos simulações de Monte Carlo
do sistema de transmissão DDO-OFDM II na configuração B2B óptico e através de 20
e 40 km de fibra SSMF, conforme a lógica definida na Tabela
\ref{Tab:pseudoCodigo}.
Os prinicpais parâmetros OFDM utilizados em todas simulações conduzidas estão
resumidos na Tabela \ref{Tab:3}.

\begin{center}
\begin{table}[h]
	\centering
	\caption{Parâmetros OFDM.}
	\begin{tabular}{c c}
		\hline
...
%		\hline
		...
		\hline
		Taxa de Transmissão (Gb/s)            & 1.83          \\
		Largura de Banda Útil (GHz)           & 1.0           \\
		Tamanho da IFFT/FFT                   & 1024          \\
		Subportadoras de Informação           & 512           \\
		Espaçamento entre Subportadoras (MHz) & 1.95          \\
        Mapeamento nas Subportadoras          & 4, 8 e 16-QAM \\
		\hline
%		\caption{AA}
	\end{tabular}
	\label{Tab:3}
...
Os resultados da otimização de parâmetros pelo GA proposto, para os mapeamentos
4, 8 e 16-QAM nas subportadoras de informação, estão ilustrados na Tabela
\ref{Tab:4}. O menor valor de OSNR para a taxa de erro de bits BER=$10^{-3}$, em
cada um dos mapeamentos em B2B, 20 e 40 km de SSMF, também são mostrados na
Tabela \ref{Tab:4}.
...
Perceba-se pelos resultados mostrados na Tabela \ref{Tab:4} que, o aumento da
eficiência espectral, dado pela redução da banda de guarda ocorreu em todos os
casos simulados. Além disso, o desempenho do sistema DDO-OFDM simulado melhorou
quando os parâmetros críticos foram otimizados. Por exemplo, o valor de $11.3$
dB de OSNR com $BER = 10^{-3}$ foi alcançado para transmissões em 20 km com a
redução de 20\% $(B_G = 0.8 \times B_W)$ na banda de guarda.
...
Para a averiguação da eficiência da codificação empregada na otimização proposta, optou-se
por realizar a comprovação experimental dos resultados obtidos ao final das simulações. 
A Figura \ref{Fig:6} ilustra o arranjo experimental preparado para a transmissão no sistema DDO-OFDM II
otimizado conforme parâmetros definidos na Tabela \ref{Tab:4}.

Os sinais OFDM elétricos parametrizados conforme mostrado na Tabela \ref{Tab:3}
foram transmitidos e processados \emph{offline} no software de computação
MATLAB\textsuperscript{\textregistered}. Para tanto, os sinais são enviados ao
gerador de onda arbitrária AWG7122C que realiza a conversão D/A. Depois do
filtro passa baixas LPF, estes são utlizados pelo modulador óptico MZM para
modular externamente o sinal óptico CW do laser de sinal centrado no comprimento
de onda $\lambda = 1550$ nm. Após propagação em fibras monomodo padrão, os
sinais ópticos são detectados por apenas um fotodetector para caracterizar a
detecção direta.

A densidade espectral de potência e a constelação recebida na configuração B2B
com a $EVM \approx -14.9$ dB, mostradas na Figura \ref{Fig:6}, confirmam a
adequação da otimização dos parâmetros através do algoritmo genético projetado.
A Figura \ref{Fig:7} mostra o desempenho para transmissões DDO-OFDM através de
20 e 40 km de SSMF sem compensação óptica da dispersão. Ressalta-se, a partir da
Figura \ref{Fig:7} que os valores de EVM iguais a -15.5 e -17.2 dB, para 20 e 40
km respectivamente, foram obtidos com as reduções da banda de guarda mostradas
na Tabela \ref{Tab:4}.
\begin{figure}[H]
	\centering
		\includegraphics[scale=0.75]{Figures/DDO-OFDM_Experimento_Labtel_out2012_JMOe.eps}
		\caption{Configuração experimental do sistema DDO-OFDM II à taxa de $R_b =
		1.83$ Gbps.}
		\label{Fig:6}
\end{figure}

Um questionamento que naturalmente surge olhando-se atentamente as constelações
das Figuras \ref{Fig:6} e \ref{Fig:7} está relacionado ao fato do desempenho da
propagação mediante 40 km de SSMF ser melhor que na transmissão em 20 km que,
por sua vez é melhor que na configuração B2B. A resposta para isto está no fato
de que para cada transmissão, os parâmetros otimizados são diferentes. Ou seja,
os parâmetros utilizados para propagação em 20 km não são otimizados a partir da
otimização realizada em B2B. A heurística inicializa a otimização em cada
configuração em separado, considerando inclusive o comprimento do enlace a
retratar.
...
primeira formulação é no máximo igual a 20\%. Porém, resultados obtidos por
\cite{art:peng} demonstram a obtenção de uma banda de guarda dada por $B_G
\approx 0.35 \times B_W$ utilizando um sinal OFDM com banda lateral virtual
(\textit{Virtual Single Side Band - OFDM} VSSB-OFDM) em um sistema óptico de
comunicação com frequência ajustável. Projetado para enlaces de fibra de longas
distâncias, sua eficiência espectral está associada a um aumento da complexidade
necessária no receptor para reduzir o SSBI no sinal desejado.

Assim, uma nova formulação do problema que considera a eficiência espectral é
abordada nesta seção a fim de aumentar eficiência espectral de todo o sistema.
Portanto, um termo $\frac{B_G}{B_W}$ é adicionado em uma nova função objetivo.
Isto significa que, no caso de duas soluções fechadas geradas (semelhante OSNR a
uma $BER = 10^{-3}$), o algoritmo genético escolherá o cromossomo com o menor
valor de $\frac{B_G}{B_W}$. Esta nova formulação e o \textit{fitness} a ela
associada estão detalhados na Tabela \ref{Tab:5}.

\begin{center}
\begin{table}[h]
\centering
\caption{Nova Formulação e \textit{Fitness} Associado.}
	\begin{tabular}{c}
		\hline
...
garante um aumento máximo na eficiência espectral do sistema como um todo
(considerando-se a banda útil na banda total) em torno de 18\%, o qual ocorre
nas transmissões em configuração B2B, para mapeamento 4-QAM nas subportadoras.
...
É possível notar que, em quase todas simulações realizadas com essa nova
formulação, os valores de OSNR encontrados para $BER = 10^{-3}$ foram
praticamente os mesmos encontrados na formulação inicial. Observa-se também
pelos resultados da Tabela \ref{Tab:6} que, $V_{\textit{bias}}$ por volta de
$0.9 \cdot V_\pi$ traduz-se em um ponto escolhido para a polarização para o MZM.

Convém ressaltar que o parâmetro $I_G$ dado pelo prefíxo cíclico não é descrito
nas Tabelas \ref{Tab:4} e \ref{Tab:6}, devido a sua insignificância na
otimização do sistema.

Uma comparação entre um sistema DDO-OFDM convencional e um sistema DDO-OFDM com
parâmetros fundamentais otimizados pelo algoritmo genético proposto na última
formulação do problema é mostrada na Figura \ref{Fig:8}.
...
Observa-se pelas curvas da Figura \ref{Fig:8} que a banda de guarda pode ser
reduzida para no máximo 50\% ($B_G = 0.5 \times B_W$) da largura de banda
efetiva do sinal em sistemas OFDM ópticos convencionais com mapeamento das
subportadoras de 4-QAM e configuração B2B, com uma penalidade na OSNR aceitável
de aproximadamente 2 dB quando comparado com 20\% de redução no sistema
convencional na mesma conjuntura. Ressalta-se que, conforme ilustrado na Figura
\ref{Fig:8}, a simulação de Monte Carlo não converge para $B_G > 0.5 \cdot B_W$.
Além disso, uma redução adicional de 20\% é fornecida pela otimização proposta
para o mesmo desempenho do sistema ($OSNR = 5.2$ dB com $BER = 10^{-3}$). Esse
ganho adicional na eficiência espectral também ocorre em transmissões de sinais
OFDM compostos por subportadoras mapeadas em 4-QAM para propagação em 20 km de
fibra SSMF. Em B2B, o pior caso ocorre no enlace de fibra de comprimento igual a
40 km onde o desempenho e a eficiência espectral do sistema são exatamente os
mesmos.

Conforme mostrado na Figura \ref{Fig:8}, a convergência da simulação MC permite
um máximo de redução na banda de guarda de 20\% para ambos os mapeamentos 8 e
16-QAM, tanto na configuração B2B, quanto na propagação em 20 e 40 km de fibra
SSMF.
Nesses casos, os ganhos na eficiência espectral fornecidos pela otimização
alcançam 8\% em um ganho de 0.5 dB no desempenho do sistema ($OSNR = 11.1 dB$ em
um $BER = 10^{-3}$ para uma mapeamento de 16-QAM com B2B).

O pior caso da otimização ocorre para um mapeamento nas portadoras em 8-QAM e
transmissão através de 20 km no qual a eficiência espectral é exatamente a mesma
registrada nos sistemas convencionais. O resultado da otimização neste caso pode
ser melhorado, aumentando-se a quantidade de iterações no processo de
otimização.

\section{Considerações Finais}
A otimização de sistemas DDO-OFDM via o algoritmo genético descrito neste
Capítulo, mostrou-se necessária devido à grande quantidade de parâmetros
envolvidos no desempenho de tais sistemas. Ganhos na relação OSNR para taxas de
um $BER = 1 \times 10^{-3}$ foram encontrados mesmo com redução na largura de
banda de guarda (aumento da eficiência espectral) que alcançaram 50\%.
...
% 
% \section{Técnica de Imposição da OSNR}
% A Figura \ref{Fig:StructureSimulated} demonstra a estrutura do sistema simulado.
% Diferentemente do sistema experimental, efeitos de ruído de emissão espontânea
% de amplificadores (\textit{Amplified Spontaneous Emission} - ASE) como fonte de
% ruído óptico predominante foram simulados por um gerador de ruído aleatório.
% Assim sendo, o ruído inserido ao sinal ótico, após a propagação na fibra é
% ponderado conforme uma relação sinal-ruído óptica imposta.
% % ASE nao sistema proposto para a otimização foi incluído um amplificador óptico
% % ideal (AO) com a inserção de ruído gaussiano antes do fotodetector. Embora a
...
% 
% \section{O Algoritmo Genético Aplicado ao Problema}
% Neste trabalho é utilizado o algoritmo genético com codificação de número real
% com um processo que inicia com uma população $P_F$ gerada de uma ordem aleatória
% por $N = 30$ soluções possíveis para $\sigma_s$, $V_{bias}$, $B_G$, $I_G$, e
% $n_{samp}$ como mostrado na Figura \ref{Fig:GA_blockDiagram}.
% Cada cromossomo tem um \textit{fitness} que avalia a qualidade de cada solução
