\chapter{Otimização da Eficiência Espectral de Sistemas DDO-OFDM II}
...
otimização na tentativa de encontrar valores que aumentam a eficiência espectral
dos sistemas. Este Capítulo enfatiza o aumento da eficiência espectral de
sistemas DDO-OFDM em enlaces de curto alcance com parâmetros otimizados por um
algoritmo Genético (\textit{Genetic algorithm} - GA) mono-objetivo. A eficiência
da otimização é mostrada perante a transmissão experimental de um sistema
DDO-OFDM II otimizado, a uma taxa de 1.83 Gb/s e 40 km de fibra SSMF sem
compensação óptica da dispersão cromática.
...
desperdiçada pela portadora óptica. Polarizar o MZM acima do ponto de quadratura
proporciona maior potência óptica da portadora na saída do modulador. Em
contrapartida, entra-se numa região de não linearidade caso ocorram picos do
sinal transmitido, ao polarizarmos o MZM na zona de máxima transferência de
potência da curva característica. Já polarizando o modulador abaixo do ponto de
quadratura e logo acima da tensão nula, reduz a perda de potência pela portadora
óptica, ao mesmo tempo em que são introduzidos termos de alta ordem provindos de
não linearidades dado também pela curva da função transferência do MZM.
\begin{figure}[ht]
\centering
	\includegraphics[width=\textwidth]{Figures/MZM_Curve_cap5.pdf}
...
\centering
	\begin{subfigure}[b]{0.9\textwidth}
        ...
        \caption{Inserção da Banda de Guarda no Sistema DDO-OFDM II;}
        \label{Fig:BGa}
	\end{subfigure}
	~	%add desired spacing between images, e. g. ~, \quad, \qquad etc. %(or a blank
	% line to force the subfigure onto a new line)
	\begin{subfigure}[b]{0.9\textwidth}
    	\includegraphics[width=\textwidth]{Figures/Graph_BG_Chap5-b.pdf}
        \caption{Espectro elétrico esperado com parâmetros otimizados.}
		\label{Fig:BGb}
	\end{subfigure}
	\caption{Variação proposta da banda de guarda do sistema DDO-OFDM II.}
	\label{Fig:BG}
\end{figure}
...
Otimização mono-objetivo significa otimizar um problema bem definido sujeito a
um conjunto de restrições, a partir de uma única função objetiva.
...
uma nova população de cromossomos é formado por meio de um mecanismo de seleção
...
elaborada para cada problema a ser resolvido. Dado um cromossomo particular, uma
...
população. O papel da mutação nos GAs é recuperar material genético perdido ou
...
Para otimizar o desempenho do sistema DDO-OFDM II simulado, a formulação do
...
esta formulação pode ser expressa como mostrada na Tabela
...
Assim sendo, o ruído inserido ao sinal óptico, após a propagação na fibra é
...
eficiência espectral de sistemas DDO-OFDM II em enlaces de curto alcance com
...
transmissão de um sistema DDO-OFDM II otimizado, a taxa de 1.83 Gb/s em
condições de B2B, 20 e 40 km de fibra SSMF sem compensação óptica de dispersão
são discutidos. Além disso, realiza-se aqui uma discussão acerca da formulação
do problema que considera a maximização da eficiência espectral do sistema como
um todo.

\begin{table}[h]
\caption{Algoritmo Genético para Sistemas DDO-OFDM II}
\centering
	\begin{tabular}{|l l|}
		\hline
		\multicolumn{2}{|c|}{\textbf{Algoritmo Genético para Sistemas DDO-OFDM II}} \\
		\hline
...
Os principais parâmetros OFDM utilizados em todas simulações conduzidas estão
...
\begin{table}[h]
\caption{Resultados da Simulação do sistema DDO-OFDM II otimizado.}
\centering
...
Percebe-se pelos resultados mostrados na Tabela \ref{Tab:4} que, o aumento da
...
Para a averiguação da eficiência da codificação empregada na otimização
proposta, optou-se por realizar a comprovação experimental dos resultados
obtidos ao final das simulações. A Figura \ref{Fig:6} ilustra o arranjo
experimental preparado para a transmissão no sistema DDO-OFDM II otimizado
conforme parâmetros definidos na Tabela \ref{Tab:4}.

Os sinais OFDM elétricos parametrizados conforme mostrado na Tabela \ref{Tab:3}
foram transmitidos e processados \emph{offline} no \textit{software} de
computação MATLAB\textsuperscript{\textregistered}. Para tanto, os sinais são
enviados ao gerador de onda arbitrária AWG7122C que realiza a conversão D/A.
Depois do filtro passa baixas LPF, estes são utlizados pelo modulador óptico MZM
para modular externamente o sinal óptico CW do \textit{laser} de sinal centrado no
comprimento de onda $\lambda = 1550$ nm. Após propagação em fibras monomodo
padrão, os sinais ópticos são detectados por apenas um fotodetector para
caracterizar a detecção direta.
...
A Figura \ref{Fig:7} mostra o desempenho para transmissões DDO-OFDM II através
de 20 e 40 km de SSMF sem compensação óptica da dispersão. Ressalta-se, a partir
da Figura \ref{Fig:7} que os valores de EVM iguais a -15.5 e -17.2 dB, para 20 e
40 km respectivamente, foram obtidos com as reduções da banda de guarda
mostradas na Tabela \ref{Tab:4}.
...
\begin{figure}[htbp]
	\centering
		\includegraphics[width=0.86\textwidth]{Figures/DDO-OFDM_Experimento_Labtel_out2012_JMOe.eps}
...
\begin{figure}[h]
	\centering
		\includegraphics[width=0.8\textwidth]{Figures/Spectrums_Constelations.eps}
...
\caption{Nova Formulação e \textit{Fitness} Associado.}
	...
	\begin{tabular}{|c|}
		\hline
		\multicolumn{1}{|c|}{\textbf{Min.} OSNR $@ BER = 10^{-3} +
		\left(\frac{B_G}{B_W}\right)$ \textbf{Sujeito a}} \\
		\hline
		$0.01 \times V_\pi \leq \sigma_S \leq 0.25 \times V_\pi$ \\
		$0.5 \times V_\pi \leq V_{bias} \leq V_{\pi} $ \\
		$0.1 \times B_W \leq B_G \leq B_W $ \\
...
$0.9 \cdot V_\pi$ traduz-se em um ponto escolhido para a polarização do MZM.
\begin{table}[hp]
\caption{Resultados da Simulação do sistema DDO-OFDM II otimizado para a
...
A otimização de sistemas DDO-OFDM II via o algoritmo Genético descrito neste
