...
alguns parâmetros do extenso número que compõem as características de formação e
transmissão do sinal OFDM, em sistemas DDO-OFDM, afeta significativamente a
qualidade da transmissão. Tais parâmetros, são responsáveis pela quantidade de
potência do sinal na entrada da fibra, pela sensibilidade do receptor, pela
robustez contra dispersão cromática da fibra e pela eficiência espectral do
sistema.
...
óptica (\textit{Optical Modulation Index} - OMI) aceitável é necessário para
superar a potência óptica do sinal (em especial a potência da portadora óptica
desperdiçada) e distorções não lineares produzidas pelo próprio MZM
\cite{WilnnerJLT09_02}. Sabe-se também que a eficiência global do sistema
DDO-OFDM é afetada pela banda de guarda $B_G$ necessária para evitar a
interferência de batimendo do sinal (\textit{signal-to-signal beat interference}
- SSBI ou \textit{subcarries-to-subcarries mixing interference} - SSMI) presente
na recepção óptica. Neste quesito em especial, a literatura diz que, para uma
banda efetiva de transmissão $B_{W}$ é necessária uma banda de guarda $B_G =
B_{W}$ para garantir que não haja interferência \cite{Rosenkranz09}. A redução
desta banda de guarda torna-se tarefa essencial dos sistemas aqui apresentados
de forma a aumentar a eficiência espectral dos mesmos.
...
otimização na tentativa de encontrar valores que aumentam as eficiências
espectrais dos sistemas.
...
mostrada com a transmissão experimental de um sistema DDO-OFDM de dupla faixa
lateral com $R_b =$ 1.83 Gb/s e 40 km de fibra sem compensação óptica da
dispersão cromática.
...
\cite{art:peng} revelam pontos relevantes de projeto entre potência do sinal
...
óptica B2B, um único símbolo OFDM pode ser escrito
como% [7]-[12].
\begin{equation}
...
é recomendada, conforme ilustra a Figura \ref{Fig:BGa}. Durante os procedimentos
experimentais descritos no capítulo \ref{Cap:4}, notou-se que a banda de guarda
$B_G$ não necessáriamente deveria seguir esta igualdade.
% o que justificou adicionar este parâmetro aos algoritmos de otimização obtendo
% valores que não causassem interferências com o sinal transmitido.

% Sendo assim, qual valor de banda de guarda deve ser atribuía em aplicação que
% para OFDM são 
